/* PAEQ-64: optimized (AES-NI) version*/
...
int PAEQ64_opt_AESNI_decrypt(unsigned char *m, unsigned long long *mlen,
	...
	const unsigned char *k)
{
	...
	__m128i lastinput_mask = _mm_set_epi32(0, 0, 0xffffffff, 0xffffffff);
	__m128i key_mask = _mm_set_epi32(0xffffffff, 0xffffffff,0,0);
	...
	//1.3.2 Nonce and key
	__m128i Key = _mm_set_epi64x(*((uint64_t*)k), 0);
	BlockInput[3] = _mm_set_epi64x(*((uint64_t*)k), (*((uint64_t*)npub)));
		...
		while ((clen>CRYPTO_ABYTES))
		{
			...
			if ((clen < (2 * CRYPTO_MBLOCK + CRYPTO_ABYTES)) || (decrypted_bytes == 0))
			{
				...
				else   //Second input is D3 || C || X || F(K), where C = P + G(K), so P = C + G(K), and X = H(K) + LLLLL, where L is the length of C
				{
					...
					boundary[3] = _mm_set_epi32(0, 0, 0xffffffff, 0xffffffff);  
			...
			else //More than 115 bytes to encrypt
			{
				...
				__m128i plaintext_buffer[8];  //96-byte storage
				...
				__m128i BlockExtraMiddleTmp = _mm_loadu_si128((__m128i*)(c + decrypted_bytes + 38));  //Bytes [38:53]
				BlockExtraMiddleTmp = _mm_srli_si128(BlockExtraMiddleTmp, 8);   //Bytes [46:53]
				...
				__m128i BlockMiddleTmp = _mm_loadu_si128((__m128i*)(c + decrypted_bytes + 38 + CRYPTO_MBLOCK));  //Bytes [38:53]
				BlockMiddleTmp = _mm_srli_si128(BlockMiddleTmp, 8);
	...
	if (adlen != 0)
	{
		...
		while (adlen>0)
		{
			if ((adlen < (2 * CRYPTO_ADBLOCK)) || (auth_bytes == 0)) //It is inconvenient or impossible to run two FPerms in parallel
			{
				...
				//3.1 Filling AD buffers (block bytes 8-55)
			...
			else //We authenticate 92-byte chunks
			{
				...
				//2. Loading AD
				__m128i first_adblock = _mm_loadu_si128((__m128i*)(ad + auth_bytes - 10)); //Loading 16 bytes of AD
				first_adblock = _mm_slli_si128(first_adblock, 10);	//Keeping only 6 bytes of AD 
				BlockExtraInput[0] = _mm_xor_si128(BlockExtraInput[0], first_adblock);  //Loading 6 bytes of AD
				first_adblock = _mm_loadu_si128((__m128i*)(ad + auth_bytes - 10 + CRYPTO_ADBLOCK)); //Loading 16 bytes of AD
				first_adblock = _mm_slli_si128(first_adblock, 10);	//Keeping only 6 bytes of AD 
				BlockInput[0] = _mm_xor_si128(BlockInput[0], first_adblock);  //Loading 6 bytes of AD

				BlockExtraInput[1] = _mm_loadu_si128((__m128i*)(ad + auth_bytes + 6));
				BlockExtraInput[2] = _mm_loadu_si128((__m128i*)(ad + auth_bytes + 22));  //Bytes [14:29]
				BlockInput[1] = _mm_loadu_si128((__m128i*)(ad + auth_bytes + 6 + CRYPTO_ADBLOCK));
				BlockInput[2] = _mm_loadu_si128((__m128i*)(ad + auth_bytes + 22 + CRYPTO_ADBLOCK));  //Bytes [14:29]

				__m128i last_adblock = _mm_loadu_si128((__m128i*)(ad + auth_bytes + 30)); //Loading 16 bytes of AD
				last_adblock = _mm_srli_si128(last_adblock, 8);	//Keeping only 8 bytes of AD 
				BlockExtraInput[3] = _mm_xor_si128(BlockExtraInput[3], last_adblock);  //Loading 8 bytes of AD
				last_adblock = _mm_loadu_si128((__m128i*)(ad + auth_bytes + 30 + CRYPTO_ADBLOCK)); //Loading 16 bytes of AD
				last_adblock = _mm_srli_si128(last_adblock, 8);	//Keeping only 8 bytes of AD 
				BlockInput[3] = _mm_xor_si128(BlockInput[3], last_adblock);  //Loading 8 bytes of AD
...

{
	...
	if (mlen != 0)
	{
		...
		while ((mlen>0))
		{
			...
			else //More than 161 bytes to encrypt
			{
				...
				plaintext_buffer[3] = _mm_loadu_si128((__m128i*)(m + encrypted_bytes + 38));  //Bytes [38:53]
				plaintext_buffer[3] = _mm_srli_si128(plaintext_buffer[3],8); //Right  shift to keep bytes [46:53] only
				plaintext_buffer[4] = _mm_loadu_si128((__m128i*)(m + encrypted_bytes + 52));  //Bytes [0:13]
				plaintext_buffer[5] = _mm_loadu_si128((__m128i*)(m + encrypted_bytes + 68));   //Bytes [14:29]
				plaintext_buffer[6] = _mm_loadu_si128((__m128i*)(m + encrypted_bytes + 84));  //Bytes [30:45]
				plaintext_buffer[7] = _mm_loadu_si128((__m128i*)(m + encrypted_bytes + 92));  //Bytes [38:53]
				plaintext_buffer[7] = _mm_srli_si128(plaintext_buffer[7],8); //Right  shift to keep bytes [46:53] only
	...
	if (adlen != 0)
	{
		...
		//Preparing the block: Constant (bytes 0-1) || Counter (bytes 2-9) || Data (bytes 10-55) || Key (bytes 56-63)
		BlockInput[0] = _mm_add_epi64(D0, four128);
		BlockInput[3] = Key;
		while (adlen>0)
		{
...
{
	return PAEQ64_opt_AESNI_encrypt(c, clen, m, mlen, ad, adlen, npub, k);
}
...
{
	return PAEQ64_opt_AESNI_decrypt(m, mlen, c, clen, ad, adlen, npub, k);
}
